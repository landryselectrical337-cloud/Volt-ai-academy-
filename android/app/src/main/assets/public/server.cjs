...
Return JSON object:
{
  ...
  "practicalApplication": "Why this matters on the job site or in business."
    ...
    const prompt = mode === "christian" ? "Generate a powerful Christian principle for an electrical contractor based on Proverbs or the New Testament regarding integrity, fair pricing, hard work, or customer trust." : mode === "quote" ? "Generate an inspiring craftsmanship and trade motivation quote for electricians." : "Generate a witty, clean, hilarious electrician trade joke or apprentice humor.";
    const response = await ai.models.generateContent({
      model: "gemini-3.6-flash",
      contents: prompt,
      config: {
